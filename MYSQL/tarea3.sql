...
where address.city_id=312; -- o where city.city_id=312, da el mismo resultado
...
-- la claificación, las características especiales y el género (categoría).

select * from category; 
select * from film;

select film.title, film.description, film.release_year, film.rating, film.special_features, category.name genre
from  film_category -- tabla de union entre tabla film y tabla category
...
select actor.actor_id, actor.first_name nombre_autor, film.title tutulo_pelicula, film.description, film.release_year año_lanzamiento
from film_actor
...
-- Su consulta debe devolver el nombre, apellido, correo electrónico y dirección del cliente.
select address.city_id, concat(customer.first_name, ' ' , customer.last_name)as nombre_apellido_cliente, customer.email, address.address
from customer
...
--  la claificación y la función especial. 
